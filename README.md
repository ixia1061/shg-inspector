...
3. [Vercel 재배포 방법](#3-vercel-재배포-방법)
...
이 프로젝트는 **GitHub 저장소에 연결된 Vercel 프로젝트**로 운영한다.
`main`(기본) 브랜치에 커밋을 push 하면 Vercel이 자동으로 빌드·배포한다 — 즉, **재배포 = git push**.
...
git add -A
git commit -m "설명"
git push            # push 즉시 Vercel이 프로덕션 빌드를 시작한다
```

Vercel 대시보드 → 프로젝트 → **Deployments** 탭에서 빌드 로그와 진행 상태를 확인할 수 있다.

### 환경 변수를 바꿨을 때

환경 변수는 코드가 아니라 Vercel에 저장되므로, 값을 바꾼 뒤에는 **재배포를 한 번 해야** 반영된다.

1. Vercel 대시보드 → 프로젝트 → **Settings → Environment Variables** 에서 값 수정
2. **Deployments** 탭 → 최신 배포의 **⋯ → Redeploy** (또는 아무 커밋이나 다시 push)

### 필요한 환경 변수 (Vercel Settings → Environment Variables)

Production/Preview 스코프에 아래 값을 등록한다. [환경 변수 레퍼런스](#환경-변수-레퍼런스) 참고.

| 변수 | 값 |
|---|---|
| `NEXT_PUBLIC_SUPABASE_URL` | Supabase Project URL |
| `NEXT_PUBLIC_SUPABASE_ANON_KEY` | anon/publishable key |
| `NEXT_PUBLIC_APP_URL` | **배포된 도메인** (예: `https://your-app.vercel.app`) |
| `SUPABASE_SERVICE_ROLE_KEY` | service_role/secret key (Sensitive로 등록 권장) |

> `NEXT_PUBLIC_APP_URL`은 QR 라벨에 인코딩되는 절대 URL의 기준이 된다.
> 로컬에서는 `http://localhost:3000`, 운영에서는 실제 배포 도메인으로 설정해야 QR이 올바른 주소를 가리킨다.

### 최초 1회 연결 (아직 GitHub·Vercel에 연결하지 않은 경우)
...
1. **GitHub 저장소 생성 후 push**
   ...
   ```
2. **Vercel에서 저장소 가져오기**: [vercel.com/new](https://vercel.com/new) → GitHub 계정 연결 → 이 저장소 **Import**
3. **Framework Preset**: Next.js (자동 감지). Build Command/Output은 기본값 그대로 둔다
   (Vercel이 `npm run build` = `next build --webpack`를 실행).
4. **Environment Variables**: 위 표의 4개 변수를 입력한다. 값은 로컬 `.env.local`에서 복사해 붙여넣되,
   `.env.local` 파일 자체는 절대 커밋하지 않는다(이미 `.gitignore`로 제외됨).
5. **Deploy** 클릭. 완료되면 발급된 도메인으로 접속.
6. (권장) 발급된 도메인을 `NEXT_PUBLIC_APP_URL`에 반영하고 한 번 재배포한다.
