...
관리자 계정은 앱에서 발급하므로, 최초 1개만 수동으로 만든다.
...
2. **SQL Editor** 에서 해당 사용자를 관리자로 승격:
   ...
   > 가입 트리거는 모든 신규 계정을 **비활성 점검자**로 만든다(자가 가입의 권한 상승 방지).
   > 그래서 `is_active = true`를 함께 켜야 로그인이 된다.

이후에는 앱의 **사용자 관리** 화면에서 관리자/점검자 계정을 추가 발급할 수 있다.

### 2-5. 점검자 가입 (가입코드 + 승인)

점검자는 관리자가 일일이 만들지 않아도 스스로 신청할 수 있다.

1. 시스템관리자가 **사용자 관리 → 관리자 목록**에서 각 관리자에게 가입코드를 발급한다(관리자당 1개, 재발급하면 옛 코드는 즉시 무효). 관리자는 사용자 관리 맨 위에서 자기 코드를 본다.
2. 점검자가 로그인 화면의 **[점검자이신가요? 가입 신청]** 에서 가입코드·이름·이메일·비밀번호를 넣고 신청한다.
3. 코드 주인 관리자가 **사용자 관리 → 가입 승인 대기**에서 승인하며, 자기 담당 사업장 중 **점검할 사업장을 체크**해 범위를 정한다.

승인 전 계정은 `is_active = false`라 로그인해도 아무 데이터에 접근할 수 없다(RLS 헬퍼가 차단).
Supabase의 공개 가입 설정은 **꺼둔 채로** 둔다 — 가입은 서버 액션이 service_role로 처리하므로
브라우저가 `signUp()`을 호출하지 않는다. 이메일 인증(SMTP) 설정도 필요 없다.
