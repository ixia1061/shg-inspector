...
- **역할 체계 (3단계)**
  ...
  - `admin`(관리자): **배정된 담당 사업장 범위 내에서만** 건물/층/구역/차량/소화기·점검·대시보드·통계 관리. 사업장 등록/수정/삭제는 불가. **사용자 관리는 자기 사업장 점검자 한정**(조회 + 점검자 생성 + 가입 승인/거부 + **활성/비활성**, 역할 변경·계정 삭제는 불가). QR 없이 목록에서 점검 가능(관리자 영역 모달).
