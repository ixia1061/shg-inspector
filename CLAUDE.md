...
- **역할 체계 (3단계)**
  ...
  - **점검자 권한은 화면에서 항상 "사업장 단위"로 다룬다**(가입 승인·점검자 생성·범위 변경 모두). 파트가 13개까지 늘어 하나씩 고르기 번거롭기 때문. 실제 DB 배정은 `user_parts`(파트 단위) 그대로라 관리자 경계를 벗어나지 않는다 — 체크한 사업장의 **내가 맡은 파트만** 펼쳐서 부여(`partIdsForSites` in `lib/utils/part.ts`).
