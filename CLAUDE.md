...
- **2026-07-18** — **상주업체 소화기 17개 일괄 입력**(2단계). 기상대(`기상`) 6·AQ(`AQ`) 2·생명푸드시스템(`식당`) 6·프리존에어포트서비스(`프리존`) 1·코드 주식회사(`코드`) 2 — org_code가 달라 각각 **별도 사업장**으로 생성. 종류: 분말 15·할론 1(기상 전산실)·K급 1(식당 주방). 이로써 전체 소화기 **479개**(본청 462 + 상주 17). 임시 Node 스크립트로 수행, 데이터라 재배포 불필요(운영 DB 즉시 반영).
