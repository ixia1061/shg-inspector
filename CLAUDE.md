...
- **2026-07-19** — **관리대장: 표지 보유현황을 종류+용량별로 세분 + 점검 이상내용 컬럼 추가.** ① 표지 보유현황 열을 종류만이 아닌 **종류·용량 조합**(예: `분말 3.3kg`, `CO2 4.6kg`)으로 세분(무안 15조합/상주 4조합). ② 점검대장에 **"점검내용(불량/조치)"** 컬럼 추가 — 최근 점검의 **불량 항목**(압력/봉인/외관/설치 불량)과 **비고(memo, 교체 등 조치 내용)**를 표시. 이를 위해 `v_extinguisher_overview`에 `last_inspection_memo`·`last_pressure_ok`·`last_seal_ok`·`last_appearance_ok`·`last_installation_ok` 노출(마이그레이션 `20260719090000`, 서울 DB 적용 완료). `Combo` 타입/`inspectionNote()` 추가.
