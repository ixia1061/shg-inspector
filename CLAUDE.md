...
- **역할 체계 (3단계)**
  - `super_admin`(시스템관리자): 모든 관리자 권한 + **사용자 추가/역할 변경 독점**, 삭제·강등·비활성 불가(보호).
  - `admin`(관리자): 사업장/건물/소화기 등 마스터데이터·대시보드·사진·통계. 사용자 관리 불가.
  - `inspector`(점검자): **QR 스캔을 통해서만** 점검 가능. 조회 전용. (관리자는 QR 없이 목록에서 점검 가능하고, 그 점검은 점검자에게도 완료로 반영.)
