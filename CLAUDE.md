...
| 테이블 | 요약 |
|---|---|
| `profiles` | auth.users 확장. `role`(super_admin/admin/inspector), **`is_active`(= 승인 플래그, RLS가 실제로 차단)**, `name`, `email`(트리거가 복사), `pending_admin_id`(가입 신청을 받은 관리자 — 승인 시 null) |
| `admin_join_codes` | **관리자별 점검자 가입코드**(관리자당 1행, 4자리). 발급·재발급은 시스템관리자만(재발급 시 옛 코드 즉시 무효), **읽기는 본인 코드만**(시스템관리자는 전부). 이 코드로 신청하면 그 관리자에게 접수된다 |
| `signup_attempts` | 가입코드 무작위 대입 차단용 시도 기록(IP·성공여부). RLS 켜고 **정책 없음** — 가입 서버 액션이 service_role로만 사용 |
...
| `management_parts` | **관리파트**. `site_id`, `code`(관리번호 앞자리, **전체 유일**), `name`, `order_index`. 사업장 하위, 소화기 등록 시 선택. **쓰기는 시스템관리자만** |
...
- `v_ledger_months` — 관리대장 보관함용. 사업장별로 **점검이 있었던 달**과 그 달 점검 대수.
...
- **리전**: `ap-northeast-2`(서울) — 속도 개선 위해 뭄바이에서 이전 완료(2026-07). **Supabase 프로젝트는 이것 하나뿐**(옛 뭄바이 프로젝트 `nppqfmcrjvipjlcqjajv`는 2026-07-30 삭제). 접속 대상은 항상 `.env.local`의 `NEXT_PUBLIC_SUPABASE_URL` ref로 확인.
- **Auth**: 이메일/비밀번호. **관리자 계정은 시스템관리자가 발급**하고, **점검자는 관리자 가입코드로 자가 신청 → 그 관리자가 사업장을 골라 승인**(2026-07-28). Supabase의 공개 가입 설정은 꺼둔 채 서버 액션이 service_role로 계정을 만든다(브라우저 `signUp()` 미사용 = 메타데이터로 role을 심을 여지 없음, SMTP 불필요). `handle_new_user` 트리거는 **항상 "비활성 점검자"로** profile을 만들고, 권한이 검증된 서버 액션만 그 뒤에 값을 올린다.
...
- **역할 체계 (3단계)**
  ...
  - `admin`(관리자): **배정된 담당 사업장 범위 내에서만** 건물/층/구역/차량/소화기·점검·대시보드·통계 관리. 사업장·관리파트 등록/수정/삭제는 불가. **사용자 관리는 자기 사업장 점검자 한정**(조회 + 점검자 생성 + 가입 승인/거부 + 점검 범위 변경 + **활성/비활성 + 삭제**, 역할 변경·담당 배정·관리자 계정 관리는 불가). QR 없이 목록에서 점검 가능(관리자 영역 모달).
  ...
  - **권한 배정**: 시스템관리자는 사용자에게 **사업장 전체(user_sites) 또는 특정 파트(user_parts)** 배정(사용자 관리). 관리자는 사용자 관리에서 **자기 담당 사업장 단위**로 점검자 범위를 바꾼다(`updateInspectorSitesAction`, RLS로 경계 강제).
  - **가입 승인**: `admin_join_codes`(**관리자당 1개**, 시스템관리자가 발급·재발급) → 점검자가 `/signup`에서 신청 → `profiles.pending_admin_id` 세팅 + `is_active=false` → **코드 주인 관리자(또는 그 범위를 포괄하는 상위 관리자)** 가 `/users`에서 승인하며 **자기 담당 사업장 중 골라** 점검 범위를 준다. 상위 판정은 `covers_admin_scope()`(코드 주인의 담당 사업장 ⊆ 내 담당 사업장), 화면 목록은 `covered_admin_ids()` RPC로 받는다. 승인/활성토글 UPDATE는 **`profiles_admin_manage_inspector`** 정책이 경계를 강제 — 대상은 점검자이면서 ①`pending_admin_id`가 나(또는 시스템관리자)거나 ②배정이 내 범위와 겹치거나 ③아직 미배정인 경우로 한정, `with check`로 역할은 점검자 고정.
  - **점검자 권한은 화면에서도 DB에서도 "사업장 단위"로 다룬다**(가입 승인·점검자 생성·범위 변경 모두). 관리자가 **사업장 전체를 담당하면 `user_sites` 한 줄**로 기록하고(2026-07-28), 파트만 담당하는 사업장은 예전처럼 `user_parts`로 기록한다 — 자기 파트보다 넓은 권한을 만들 수 없어야 하기 때문. 분기는 `splitScopeForGrant`(`app/(admin)/users/actions.ts`), DB 경계는 `user_sites_admin_grant_inspector` 정책(`has_whole_site()` 기준)이 강제.
...
- [ ] **비밀번호 찾기(셀프 재설정)** — 로그인 화면에 "비밀번호를 잊으셨나요?" + 이메일 재설정. **SMTP 연결이 선행 조건**. (2026-07-28부터 관리자가 사용자 관리에서 임시 비밀번호를 발급하는 경로가 있어 급한 불은 꺼진 상태)
...
- **2026-07-30** — **옛 뭄바이 Supabase 프로젝트 삭제(이제 프로젝트는 서울 하나뿐).** 대시보드에 죽은 DB가 계속 보여 헷갈린다는 판단 → 삭제. 실제로 2026-07-23에 이 ref로 마이그레이션을 돌려 실패한 전례가 있어, 경고 문구로 관리하기보다 대상 자체를 없애는 편이 안전하다고 봤다. **삭제 전 참조처 4곳 확인**: `.env.local`·저장소 코드·Vercel Production·Preview 전부 서울(`zbdvuxzoahusdrpniuwz`). Production은 대시보드를 믿지 않고 **배포된 번들에서 직접 확인**(`NEXT_PUBLIC_*`는 빌드 시점에 박히므로 실제 배포 파일이 곧 환경변수의 실효값 — `/_next/static/chunks/app/(auth)/login/page-*.js`에서 서울 ref 확인). 무료 플랜이라 백업 다운로드가 막혀 있었지만, 뭄바이 내용은 리전 이전 시점(2026-07)에서 멈춰 있어 이후 입력한 실데이터(무안 462·상주 17·보안 17·남부 251)가 전부 서울에만 있고 스키마는 `supabase/migrations/`에 그대로 있어 백업 없이 진행. 문서의 "뭄바이에 적용 금지" 경고를 "프로젝트는 서울 하나뿐"으로 교체(과거 변경 이력의 뭄바이 언급은 기록이므로 그대로 둠).
- **2026-07-28** — **관리대장 보관함(`/ledgers`) 분리 — 점검현황은 이번 달만.** 직전에 점검현황 다운로드 버튼 옆에 월 선택 드롭다운을 붙였는데, "점검현황은 그 달 진행 중인 내용만 받게 하고 끝난 달 대장은 별도 페이지에서 사업장별로 고르게 하자"는 요청 → **드롭다운 제거**(점검현황은 다시 현재 상태 대장만, 화면 내용과 일치) + **신규 메뉴 `관리대장`**. 보관함은 점검 기록이 있는 달을 최신 순으로 나열하고 각 달의 **점검 대수/전체 대수(점검률)** 를 함께 보여줘 어떤 달인지 바로 알 수 있게 함. 진행 중인 이번 달에는 **"진행 중" 배지**를 붙여 아직 확정이 아님을 표시(목록에서 빼면 지금은 화면이 비어 혼란스러워 표시하는 쪽을 택함). 사업장 전환은 기존 `SiteFilterButtons` 재사용, 순서는 `user_site_order` 개인 설정. 신규 뷰 `v_ledger_months`(마이그레이션 `20260728120000`, `security_invoker`라 RLS로 담당 사업장만) — 점검을 소화기→건물→사업장으로 이어 월 단위 집계. `LedgerDownloadButton`은 `month`·`label` 선택 프로프를 받아 두 화면이 공용. **서울 운영 DB 적용 완료**, 관리자별 가시 범위 검증(남부 관리자=남부공항서비스 1건 / 소방대·시스템관리자=3개 사업장). `tsc --noEmit`·`next build --webpack` 통과, lint 신규 없음.
- **2026-07-28** — **관리대장을 월별로 받을 수 있게 추가.** 질문("다음달로 넘어가면 관리대장도 초기화되나?")에서 출발 — 대장은 `v_extinguisher_overview`의 **최근 점검 1건** 기준이라, 8월에 새로 점검하면 7월 내용이 대장에서 사라진다(DB에는 남지만). 매달 말 챙겨 받지 않으면 그 달 O/X·불량내용을 재현할 수 없어서, 점검현황의 다운로드 버튼 옆에 **월 선택 드롭다운**(현재 상태 + 지난 12개월)을 추가. `GET /api/ledger/download?site=..&month=YYYY-MM`. **지난달을 고르면** `applyMonthSnapshot`이 그 달 점검을 직접 조회해 각 소화기의 **그 달 마지막 점검**으로 `last_*`·`inspected_this_month`를 바꿔 끼우고(그 달 점검이 없으면 전부 null → "미점검"), 조치 기록도 그 점검에 달린 것만 가져온다. 표지 제목·상태 컬럼명·파일명에 해당 월 표기(`소화기관리대장_한국공항공사_2026-07.xlsx`). 이번달이나 month 미지정이면 기존 동작 그대로. **내용연수 상태만은 다운로드 시점 기준**(제조일 기반 계산이라 과거 시점 재현은 별도 작업 — 실무상 현재 교체 시급도가 더 유용해 그대로 둠). 실데이터 검증: 2026-07 기준 33대 점검·나머지 717대 미점검, 점검일·점검자·결과가 DB와 일치. `tsc --noEmit`·`next build --webpack` 통과, lint 신규 없음.
- **2026-07-28** — **관리자에게 점검자 계정 삭제 권한 부여.** `deleteUserAction`을 `assertSuperAdmin`→`assertAdmin`으로 바꾸고, 일반 관리자 경로에 **대상이 점검자 + 내 범위**(`assertInspectorInMyScope`) 검증을 추가. 기존 보호는 전부 유지 — 본인 계정·시스템관리자 계정 삭제 불가, **점검 이력이 있으면 삭제 거부하고 비활성을 안내**(`inspections.inspector_id`가 `on delete restrict`). 관리자 계정 삭제와 역할 변경·담당 배정은 그대로 시스템관리자 전용. 되돌릴 수 없는 작업이라 확인 문구에 "잠시 막아두려면 비활성을 쓰라"는 안내를 넣음. UI는 `canManage`(시스템관리자)와 별도로 `canDelete` 프로프를 두어 점검자 행에만 삭제 버튼을 노출. 실데이터로 경계 확인: 남부 관리자→"남부"만 삭제 가능(A~D조는 범위밖), 소방대 관리자→A~D조는 **점검 이력 1~4건이라 삭제 차단**되고 "남부"만 가능. `tsc --noEmit`·`next build --webpack` 통과, lint 신규 없음.
- **2026-07-28** — **가입코드 조회를 관리자로 한정(점검자에게 열려 있던 것 차단).** "관리자가 가입코드를 어디서 보냐"는 질문에 답하며 실제 권한을 계정별로 확인하다 발견 — `site_join_codes_select` 정책이 `has_site_access(site_id)`만 봐서 **그 사업장 점검자도 코드를 읽을 수 있었다**(화면에는 안 나오지만 API로는 읽힘). 코드는 관리자가 현장 점검자에게 전달하는 용도이고 "관리자만 볼 수 있게" 정한 방침과도 어긋나 `is_admin() AND has_site_access(site_id)`로 좁혔다(마이그레이션 `20260728110000`). 코드를 알아도 관리자 승인이 필요하므로 실제 피해 경로는 아니었지만, 최소권한 원칙에 맞춰 정리. **서울 운영 DB 적용 완료** — 적용 후 점검자 5명 전원 조회 0건, 관리자는 담당 범위 그대로임을 확인(남부 관리자=남부공항서비스 1곳 / 소방대 관리자·시스템관리자=4곳).
- **2026-07-29** — **버그: 점검·조치 시각이 9시간 이르게(새벽처럼) 표시됨.** 제보: 점검 이력의 점검 시각·조치 시각이 UTC처럼 찍혀 새벽에 점검한 것처럼 보인다. DB는 `timestamptz`로 정확히 저장하고 있었고 **표시만 틀렸다** — `toLocaleString("ko-KR")`에 `timeZone`을 안 주면 실행 환경 시간대를 따르는데, `InspectionHistoryTimeline`이 **서버 컴포넌트**라 Vercel(UTC)에서 렌더링됐다(오전 8:01 점검 → 전날 오후 11:01로 표시). 클라이언트 컴포넌트도 기기 시간대가 어긋나면 같은 문제라, 공용 유틸 **`lib/utils/datetime.ts`(`formatKstDate`·`formatKstDateTime`)** 를 만들어 화면의 날짜·시각 표기 7곳을 전부 KST 고정으로 교체(점검 이력·소화기 목록·가입 승인 대기·사진 관리·조치완료 목록·점검자 현황·점검 화면). 이미 `timeZone`을 주고 있던 `ResolvedActionList`도 같은 유틸로 통일. 집계·경계 계산은 원래 `fn_kst_today()`·`at time zone 'Asia/Seoul'` 기반이라 영향 없음. 함께: **점검 이력·조치 내역은 대시보드처럼 날짜만** 표시(시·분 제거), 그리고 UTC로 잘라 쓰던 **파일·폴더명 날짜**도 KST로 교체(`kstDateKey`) — 특히 사진 ZIP 안의 날짜 폴더가 00:00~09:00 점검을 하루 이르게 묶고 있었다. 최종 확인: 화면에서 시간대를 지정하지 않은 `toLocale*` 호출 **0건**, 남은 `toISOString()`은 전부 저장·조회용 시각(점검 시각·조회 구간)이라 시간대와 무관. `tsc --noEmit`·`next build --webpack` 통과, lint 신규 없음.
- **2026-07-29** — **소화기 점검 이력에 불량항목·조치내역 표시(이상만 남고 처리 흔적이 없던 문제).** 제보: 소화기 상세의 점검 이력에 "이상"만 남아, 나중에 그 소화기를 볼 때 **어떻게 조치했는지 알 수 없다**. 조치는 `inspection_actions`에 잘 저장돼 있었는데(실데이터 이상 7건 중 3건 조치완료) 상세 페이지가 `inspections`만 조회해 화면에 안 나왔을 뿐이었다. **개선**: 점검 이력이 ① **조치완료/조치필요 배지**, ② **불량항목**(체크가 풀린 점검사항 — `defectItemsTextOfInspection` 신설, 뷰의 `last_*`가 아니라 `inspections` 원본 컬럼 기준이며 2026-07-27 이전 4개 항목도 함께 표기), ③ **조치내용·조치일·조치자**를 함께 보여준다. 상세 페이지가 `inspection_actions`와 조치자 이름을 함께 조회하도록 하고, memo는 "불량내용"으로 라벨을 붙여 조치내용과 구분했다. `tsc --noEmit`·`next build --webpack` 통과, lint 신규 없음.
- **2026-07-28** — **상세 화면에 뒤로가기 추가 + 소화기 목록 상태를 주소에 보존.** PC에서 쓰다 보니 사업장 상세·소화기 상세에서 목록으로 돌아갈 버튼이 없어 사이드바 메뉴를 다시 눌러야 했고, 소화기는 **4~5페이지를 보다 상세에 들어갔다 오면 1페이지로 초기화**돼 불편하다는 제보. ① `BackButton`(기존 컴포넌트 재사용)을 사업장 상세·소화기 상세·QR 라벨 화면에 추가(라벨은 `print:hidden`). ② 소화기 목록의 사업장·파트·상태·검색어·페이지를 **주소 쿼리에 남긴다** — `window.history.replaceState`라 Next 라우팅을 타지 않아 "서버 왕복 없는 즉시 필터"(2026-07-18)는 그대로고, 상세에서 뒤로 오면 브라우저가 복원한 주소를 서버 컴포넌트가 읽어 `initial`로 넘겨 보던 페이지·필터가 살아난다. `tsc --noEmit`·`next build --webpack` 통과, lint 신규 없음.
- **2026-07-28** — **버그: 상위 관리자가 하위 관리자의 가입 신청을 못 봄.** 제보: 남부 관리자 코드로 가입한 점검자가 **전체 사업장 권한이 있는 소방대 관리자**의 승인 대기에 안 보이고 시스템관리자에게만 보임. 원인은 설계 선택 — 관리자별 가입코드로 바꾸면서 대기 목록을 `pending_admin_id === 내 id`로만 걸어, 요청했던 계층(시스템관리자 > 전체 사업장 관리자 > 한 사업장 관리자)과 어긋났다. **규칙 교체**: **코드 주인의 담당 사업장이 내 담당 사업장에 전부 포함되면** 그 신청을 보고 처리할 수 있다(자기 코드는 자기 범위에 포함되므로 그대로 보임, 역방향은 차단). 신규 `covers_admin_scope()`·`covered_admin_ids()`(둘 다 security definer — 관리자는 다른 관리자의 `user_sites`를 RLS로 못 읽어 앱에서 계산할 수 없다) + `profiles_admin_manage_inspector` 정책의 대기 신청 갈래를 이 기준으로 교체(마이그레이션 `20260728170000`). 화면·승인/거부 액션도 같은 규칙 사용. **서울 운영 DB 적용 완료**, 관리자 3명 × 코드 주인 3명 전 조합을 실제 UPDATE로 검증(남부→자기 것만 / 소방대→남부+자기 / 시스템관리자→전부). 역할은 추가하지 않고 **배정 범위만으로 계층이 나온다.**
- **2026-07-28** — **관리파트 권한 개방을 취소(시스템관리자 전용으로 복귀).** 바로 아래 항목으로 열었다가 사용자 판단으로 되돌림 — 파트 코드는 관리번호(부착된 QR 라벨)와 직결되는 값이라 한 곳에서 통제하는 편이 안전하다는 결론. 마이그레이션 `20260728160000`이 `management_parts_admin_write` 정책만 drop하고(기존 마이그레이션은 수정하지 않음), 화면은 `canManageParts`를 걷어내 다시 `canManageSite`(시스템관리자) 기준으로 되돌렸다. 읽기 정책은 그대로라 관리자는 파트를 **보기만** 한다. **서울 운영 DB 적용 완료**, 실제 INSERT 시도로 관리자 2명 모두 차단됨을 확인.
- **2026-07-28** — **일반 관리자도 관리파트를 추가·수정할 수 있게 개방(같은 날 위 항목으로 취소됨).** 파트는 현장 조직이 바뀔 때마다 손봐야 하는 값이라 담당 관리자가 직접 다루는 편이 맞다는 요청. 신규 정책 `management_parts_admin_write`(마이그레이션 `20260728150000`) — 조건을 `has_site_access`가 아니라 **`has_whole_site(site_id)`** 로 잡았다. 파트 코드를 바꾸면 `fn_cascade_asset_code_from_part`(security definer)가 그 파트 소화기의 **관리번호를 연쇄 재계산**하는데, 사업장 일부(특정 파트)만 담당하는 관리자에게 열면 남의 파트 코드를 바꿔 다른 팀 소화기 관리번호까지 흔들 수 있기 때문. 화면도 같은 기준으로 `canManageSite`(사업장 수정·삭제, 시스템관리자)와 **`canManageParts`**(파트 CRUD)를 분리. **서울 운영 DB 적용 완료**, 실제 INSERT 시도로 검증(남부 관리자=남부공항서비스만 / 소방대·시스템관리자=4곳 / 점검자=차단). `tsc --noEmit`·`next build --webpack` 통과, lint 신규 없음.
- **2026-07-28** — **관리자가 임시 비밀번호를 발급하는 재설정 경로 추가.** 질문: 점검자가 개인 이메일·비밀번호로 가입하는데 비밀번호를 잊으면 어떻게 하나 — 확인해 보니 **앱 안에 재설정 수단이 전혀 없었다**(Supabase 대시보드 직접 접속뿐). 이메일 재설정 링크는 SMTP가 선행 조건이라, 지금 구조에 맞는 **관리자 임시 비밀번호 발급**을 넣었다. `resetPasswordAction`이 service_role로 `updateUserById`를 호출해 10자리 임시 비밀번호(혼동 없는 문자)를 만들어 **반환값으로만** 넘기고, `ResetPasswordButton`이 다이얼로그에 한 번만 표시(복사 버튼). 받은 사람은 로그인 후 기존 "내 계정"에서 스스로 변경. 경계: 일반 관리자는 **자기 범위 점검자만**(`assertInspectorInMyScope`), 시스템관리자는 관리자 계정까지, **다른 시스템관리자 계정과 본인 계정은 거부**(본인은 내 계정에서). 도움말·매뉴얼·FAQ에 "비밀번호를 잊었을 때" 안내 추가, TODO의 셀프 재설정 항목은 SMTP 선행 조건을 명시해 남겨둠. `tsc --noEmit`·`next build --webpack` 통과, lint 신규 없음.
- **2026-07-28** — **가입코드를 사업장별 → 관리자별로 전환.** 요청: 여러 사업장을 담당하는 관리자(소방대=4곳)가 자기에게 온 점검자에게 그중 어디를 맡길지 고를 수 있어야 하는데, 코드가 사업장에 묶여 있어 "한국공항공사 코드로 온 사람 = 한국공항공사만"으로 고정됐다. → 코드를 **관리자에게** 붙인다. `site_join_codes` 폐기, 신규 **`admin_join_codes`**(관리자당 1행, 읽기는 본인 코드만/시스템관리자 전부, 쓰기는 시스템관리자). `profiles.pending_site_id` → **`pending_admin_id`**(옛 컬럼을 참조하는 정책을 먼저 drop해야 컬럼이 지워진다 — 첫 시도에서 걸림). 승인 창은 **내 담당 사업장 체크박스**(1곳이면 자동 체크, 0곳 선택 시 승인 버튼 비활성)로 바꾸고 `approveSignupAction(userId, siteIds)`가 `pending_admin_id`가 나인지 + 고른 사업장이 내 범위인지 검증 후 기존 `splitScopeForGrant`로 기록. **가입코드 위치를 사업장 상세 → 사용자 관리로 이동**: 화면 맨 위 "내 가입코드"(모든 관리자, 복사) + 관리자 목록의 가입코드 열(시스템관리자만 발급·재발급, `issueJoinCodeAction`). **권한 계층은 새 역할 없이 그대로** — 시스템관리자 > 여러 사업장 관리자 > 한 사업장 관리자는 배정 범위에서 이미 나오고, 이번에 가입 경로도 같은 원리를 따르게 됐다. 마이그레이션 `20260728140000_admin_join_codes.sql` **서울 운영 DB 적용 완료**(리허설→검증→커밋), 코드 조회 범위 검증(남부/소방대 관리자=자기 것 1건 / 시스템관리자=3건 / 점검자=0건). `tsc --noEmit`·`next build --webpack` 통과, lint 신규 없음.
- **2026-07-28** — **관리자가 준 점검 범위를 파트 나열이 아니라 사업장 한 줄로 기록.** 요청: 담당 범위를 넓힐 때 관리파트별로 다 들어가지 말고 사업장만 기록되게. 지금까지는 `user_sites` 쓰기가 시스템관리자 전용이라 관리자가 사업장을 체크하면 그 사업장 파트를 전부 `user_parts`에 펼쳐 넣었고(남부공항서비스는 한 번에 6행), 담당 범위 표기도 "○○ 기계, ○○ 전기, …"로 길어졌다. **해결**: 신규 마이그레이션 `20260728130000_admin_grant_whole_site.sql` — `has_whole_site()`(security definer, `user_sites` 정책 안에서 `user_sites`를 조회하면 RLS가 재귀하므로 definer로 감쌈) + **`user_sites_admin_grant_inspector`** 정책으로 "관리자가 **자기가 통째로 가진 사업장**을 **점검자에게만**" 주고 뺄 수 있게 열었다. 앱은 `splitScopeForGrant`가 고른 사업장을 **내가 전체를 가진 곳 → `user_sites` 한 줄 / 파트만 담당하는 곳 → 기존대로 `user_parts`** 로 나눠 기록(파트만 담당하는 관리자가 자기보다 넓은 권한을 만들 수 없게). 가입 승인·점검자 생성·범위 변경 세 경로 모두 이 함수를 쓴다. 범위 변경 창의 "전체 배정됨" 잠금은 **내가 담당하지 않는 사업장일 때만** 걸리도록 수정(관리자가 준 사업장은 다시 해제 가능). `createUserAction`·`approveSignupAction`에서 partIds 인자를 없애고 사업장 기준으로 단순화, 승인 시에는 신청 사업장 전체를 부여. **서울 운영 DB 적용 완료** — 정책 기준 실제 삽입 시도로 검증(남부 관리자=남부공항서비스만 / 소방대·시스템관리자=4개 사업장 전부). `tsc --noEmit`·`next build --webpack` 통과, lint 신규 없음.
- **2026-07-28** — **점검자 배정 메뉴를 사용자 관리로 통합(중복 화면 제거).** 지적: 사용자 관리에 이미 담당 범위가 보이는데 점검 권한만 따로 `/assignments`에서 체크하는 건 중복이고 비효율. → 사용자 관리 목록의 담당 범위 아래 **[범위 변경]** 버튼을 두어 그 자리에서 사업장 체크로 바꾸도록 하고(`InspectorScopeDialog` + `updateInspectorSitesAction`), **`/assignments` 페이지·액션·`InspectorPartAssignments` 컴포넌트와 사이드바 메뉴를 삭제**했다. 액션은 **내가 부여 가능한 파트만 대상으로 지우고 채우므로 다른 관리자가 준 범위는 건드리지 않는다**(RLS 사용자 클라이언트 사용 → `user_parts_write` 정책이 경계 강제). 시스템관리자가 `user_sites`로 준 "사업장 전체"는 **전체 배정됨** 배지로 잠가 관리자가 풀 수 없게 유지(기존 `/assignments`의 규칙을 그대로 옮김). 시스템관리자는 기존 `UserSitesDialog`(사업장 전체/특정 파트)를 그대로 쓴다. 이로써 점검자 계정의 **생성·가입승인·범위변경·활성** 이 한 화면에 모였다. 도움말·매뉴얼의 "점검자 배정" 안내를 전부 교체. `tsc --noEmit`·`next build --webpack` 통과, lint 신규 없음.
- **2026-07-28** — **점검자 권한 지정을 사업장 단위로 통일 + 관리자에게 점검자 활성/비활성 권한 부여.** 요청: 관리파트별로 고르면 양이 방대하니 사업장 단위로, 그리고 관리자가 점검자 계정을 껐다 켤 수 있게. **① 범위 선택**: 가입 승인 다이얼로그의 파트 체크박스를 없애고 **"{사업장} 전체"** 부여로 단순화(신청 사업장이 이미 정해져 있으므로 고를 것이 없다), 관리자의 **점검자 추가**는 파트 목록 → **내가 담당하는 사업장 체크박스**로 교체. 실제 DB 배정은 `user_parts` 그대로이고 체크한 사업장의 **내가 맡은 파트만** 펼쳐 넘기므로(`partIdsForSites`) 경계는 그대로다 — 소방대 관리자(4개 사업장)는 4개가 보이고, 남부 관리자는 남부공항서비스 1개만 보인다. **② 활성/비활성**: `toggleUserActiveAction`을 `assertSuperAdmin`→`assertAdmin`으로 바꾸고, 일반 관리자 경로에는 **대상이 점검자 + 내 범위**(배정이 겹치거나 미배정) 검증을 추가(`assertInspectorInMyScope`). 계정 삭제·역할 변경은 그대로 시스템관리자 전용. 마이그레이션 `20260728100000_admin_manage_inspector.sql`이 `profiles_admin_approve_signup` 정책을 **`profiles_admin_manage_inspector`**(대기 신청 + 범위 겹침 + 미배정 3가지)로 교체해 DB에서도 같은 경계를 강제. **서울 운영 DB 적용 완료**, 정책 기준 실제 판정 확인(남부 관리자→"남부" 1명 / 소방대 관리자·시스템관리자→점검자 5명 전원). `tsc --noEmit`·`next build --webpack` 통과, lint 신규 없음.
- **2026-07-28** — **가입코드를 4자리로 줄이고, 담당 관리자도 코드를 볼 수 있게 + 무작위 대입 차단.** 현장에서 구두로 전달하기엔 8자리가 길다는 요청 → **영문+숫자 4자리**(혼동 없는 32자, 약 105만 조합). 짧아진 만큼 자동 스크립트가 찍어볼 여지가 생겨, 가입 액션에 **같은 IP 기준 10분 내 실패 10회면 차단**을 추가(신규 `signup_attempts`, RLS 켜고 정책 없이 service_role만 사용 — 앱이 직접 읽고 쓰지 않는 기록용 테이블). 또 코드가 시스템관리자에게만 보여 현장 전달이 번거로웠던 것을, **담당 사업장 관리자도 코드를 보고 복사**할 수 있게 열었다(발급·재발급은 그대로 시스템관리자만 — `JoinCodeCard`에 `canIssue` 분기, 읽기는 기존 `site_join_codes_select` 정책이 이미 `has_site_access`라 마이그레이션 불필요). 마이그레이션 `20260728090000_signup_attempt_throttle.sql`. **이미 발급해 둔 8자리 코드는 그대로 동작**하며, 재발급을 누르면 4자리로 바뀐다. `tsc --noEmit`·`next build --webpack` 통과, lint 신규 없음.
- **2026-07-27** — **점검자 자가 회원가입(사업장 가입코드) + 관리자 승인 도입, 사용자 관리를 관리자에게 개방.** 사업장 4곳·관리파트 13개로 늘면서 점검자 계정 발급이 시스템관리자 한 사람에게 몰려 병목이 됐다는 요청. **흐름**: 시스템관리자가 사업장 상세에서 **가입코드 발급**(`site_join_codes`, 사업장당 1개, 재발급 시 옛 코드 즉시 무효, 혼동 없는 32자 알파벳 4자리) → 점검자가 `/signup`에서 **가입코드·이름·이메일·비밀번호**로 신청 → 그 사업장 **관리자가 `/users`에서 승인하며 관리파트 지정** → 그때부터 로그인·점검 가능. **이메일 인증번호는 넣지 않음**(Supabase 기본 메일러가 시간당 2~3통 제한이라 현장에서 못 쓰고 외부 SMTP는 별도 작업 — 가입코드+승인으로 갈음, 이메일은 로그인 아이디). **가입은 브라우저 `signUp()`이 아니라 서버 액션 + service_role**(`admin.auth.admin.createUser`)로 처리 → Supabase 공개 가입 설정을 **꺼둔 채**로 두고, 메타데이터로 role을 심을 여지도 없앰. **함께 막은 기존 구멍 2건**: ① `handle_new_user`가 `raw_user_meta_data->>'role'`을 무검증 신뢰해 가입 화면을 여는 순간 누구나 관리자가 될 수 있었음 → **항상 "비활성 점검자"로 생성**하도록 재작성(권한이 검증된 서버 액션만 값을 올림, `createUserAction`에 `is_active: true` 추가). ② `is_active`가 로그인·RLS 어디에서도 검사되지 않아 표시용 플래그였음 → `is_active_user()` 신설 + `is_admin`·`is_super_admin`·`has_site_access`·`has_part_access`에 반영, 로그인 폼·루트·양쪽 레이아웃에서 `/login?status=pending`으로 차단. **사용자 관리 개편**: 가드를 `super_admin`→`isAdminRole`로 완화하고 **관리자는 자기 사업장 점검자만**(조회+생성+승인/거부, 역할변경·삭제·비활성 불가), **시스템관리자는 사업장 버튼 필터 + 관리자/점검자/승인대기 구분**. 별도 승인 메뉴는 만들지 않고 사용자 관리에 통합. 승인 UPDATE는 `profiles_admin_approve_signup` 정책이 DB에서도 경계 강제. `profiles`에 `email`(트리거가 복사, 기존 8명 백필)·`pending_site_id` 추가. 마이그레이션 `20260727160000_self_signup_approval.sql` **서울 운영 DB 적용 완료** — 적용 후 계정 8개 전원의 접근 범위가 이전과 동일함을 RLS 시뮬레이션으로 검증(남부 관리자 251·소방대 관리자 750·A~D조 482·남부 251·시스템관리자 750), 비활성 전환 시 소화기 0건·자기 프로필만 조회됨도 확인. 신규: `SignupForm`·`JoinCodeCard`·`PendingSignupRow`·`UsersClient`. `tsc --noEmit`·`next build --webpack` 통과, lint 신규 없음.
