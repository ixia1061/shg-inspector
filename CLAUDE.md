...
| `site_join_codes` | 사업장별 **점검자 가입코드**(사업장당 1행, 4자리). 발급·재발급은 시스템관리자만(재발급 시 옛 코드 즉시 무효), **읽기는 담당 관리자까지**(`is_admin() AND has_site_access` — 점검자는 못 읽는다) |
...
- **역할 체계 (3단계)**
  ...
  - **가입 승인**: `site_join_codes`(사업장당 1개, 시스템관리자가 발급·재발급) → 점검자가 `/signup`에서 신청 → `profiles.pending_site_id` 세팅 + `is_active=false` → 그 사업장 관리자가 `/users`에서 승인. 승인/활성토글 UPDATE는 **`profiles_admin_manage_inspector`** 정책이 경계를 강제 — 대상은 점검자이면서 ①내 사업장에 신청했거나 ②배정이 내 범위와 겹치거나 ③아직 미배정인 경우로 한정, `with check`로 역할은 점검자 고정.
