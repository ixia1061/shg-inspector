...
| `management_parts` | **관리파트**. `site_id`, `code`(관리번호 앞자리, **전체 유일**), `name`, `order_index`. 사업장 하위, 소화기 등록 시 선택. 쓰기는 시스템관리자 + **그 사업장 전체를 담당하는 관리자**(2026-07-28) |
...
- **역할 체계 (3단계)**
  ...
  - `admin`(관리자): **배정된 담당 사업장 범위 내에서만** 건물/층/구역/차량/소화기·점검·대시보드·통계 관리. **사업장 전체를 담당하면 그 사업장의 관리파트 CRUD도 가능**. 사업장 등록/수정/삭제는 불가. **사용자 관리는 자기 사업장 점검자 한정**(조회 + 점검자 생성 + 가입 승인/거부 + 점검 범위 변경 + **활성/비활성 + 삭제**, 역할 변경·담당 배정·관리자 계정 관리는 불가). QR 없이 목록에서 점검 가능(관리자 영역 모달).
